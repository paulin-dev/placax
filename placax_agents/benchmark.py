...
class Benchmark:
    ...
    def load(
        ...
        # a source format that carries one, e.g. Bookshelf's .pl file) so the canvas matches the actual
        # chip and leaves real room for whatever this run's macro_budget left out - standard cells and any
        # macros beyond the budget - the same way MaskPlace's own environment does. Only fall back to
