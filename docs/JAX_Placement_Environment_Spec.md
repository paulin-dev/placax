...
**v4.2 additions:** `resnet_cnn.py` gained `build_pretrained_resnet_backbone()` (real ImageNet weights, with a `ckpt_dir` cache/weights-path story) and `extract_resnet_backbone_weights()`/`load_resnet_backbone_weights()` (save/load *any* local backbone checkpoint into an already-initialized policy, independent of flaxmodels' own download mechanism). `ppo_loss` was generalized to read each transition's own `obs["current_macro_size"]` instead of indexing a `sizes_array` positionally by trajectory step — the latter silently assumed "trajectory position i places macro i," true for one temporally-ordered episode but not for a shuffled minibatch. That generalization is what makes the new `training/loops/buffered_train.py` (`train_buffered`) correct: a third training-loop shape — multi-episode buffer + minibatch-epoch updates — matching MaskPlace's own PPO procedure. `scripts/run_maskplace.py` assembles all of this into one runnable script, which in turn surfaced that action masking (`ExtraIllegalFn`, Section 5.2) wasn't actually threaded through `collect_rollout`/`ppo_loss`/`evaluate` yet — it is now.

**v4.3 additions:** `training/algorithm/split_optimizer.py` — `make_grouped_optimizer`/`label_params_by_name_prefix`, a generic "different optax transform per named parameter group" mechanism (thin wrapper over `optax.multi_transform`), plus `maskplace_optimizer()` (`training/algorithm/config.py`) built on it. Closes the last mechanism-diff item: MaskPlace optimizes two fully independent networks (separate Adam optimizers, separate gradient clipping, separate backward passes); placax's architectures share one trunk and take one joint gradient step. The fix isn't a second backward pass - see Section 5.1c for why a single joint `jax.grad` already gives mathematically identical per-group gradients when the groups share no parameters, which `ResNetCoarseFineActorCritic(critic_style="step_embedding")`'s critic already doesn't.
...
placax_tools/                    # Cell placer / validator wrappers (Section 5.4-5.5)
    ...
    openroad/cell_placer.py         OpenROADCellPlacer - pins, global + detailed placement
    openroad/runner.py              one Tcl script through OpenROAD, locally or in Docker
...
scripts/                         # Tier 3 — what changes per experiment (listed in full: this
    ...
    make_orfs_benchmark.py          an ORFS design (real PDK) -> a DEF benchmark + its physical stack
...
### 5.1c Per-parameter-group optimization — a fourth axis, added v4.3

Every training loop so far took one `optax.GradientTransformation` applied to the whole variable tree via one `apply_gradient_update` call — correct for a shared-trunk architecture, but not what MaskPlace does: it optimizes `actor_net` and `critic_net` as two fully separate modules, each with its own Adam optimizer, its own gradient-norm clip, and its own `.backward()` call.

`training/algorithm/split_optimizer.py`'s `make_grouped_optimizer(matched_transform, default_transform, prefix)` (built on `optax.multi_transform`) applies a different `optax.GradientTransformation` to every parameter whose name is tagged with `prefix` and a different one to everything else — each group gets its own optimizer state (its own Adam moments), independent of the other's gradients. This needed **no change to `ppo_loss` or `apply_gradient_update`** — the key realization: for two parameter groups that share no parameters, one joint `jax.grad()` call over a combined loss already produces the exact same per-group gradients that two fully separate backward passes would (basic chain rule — a loss term that doesn't depend on a parameter has zero gradient with respect to it). What differs between "one optimizer" and "two separate optimizers" isn't the gradient computation, it's what happens to each group's gradient afterward — exactly what `make_grouped_optimizer` lets you control per group, without touching the loss or the training loop at all.
...
| `oriented_grid` | `(x, y, turn)` | every macro placed | `genetic`; `ppo` with `POLICIES["oriented_cnn"]` |
...
|---|---|---|
| MaskPlace | CNN + PPO, sequential | Yes — and as of v4.3, its specific *mechanism*, not just "a CNN+PPO agent fits," composes entirely from shipped pieces, run end to end by `scripts/run_maskplace.py`: `dense=True` reward, `connectivity_order`, `macro_budget` (its `--pnm`), wirelength-guided action masking (`make_wiremask_observation` + `make_wiremask_quality_illegal`, threaded through every training loop via `ExtraIllegalFn`), a 2-macro lookahead wiremask, `maskplace_ppo_config()` for its GAE/entropy/value-loss choices, `ResNetCoarseFineActorCritic(critic_style="step_embedding")` for its network, `maskplace_optimizer()` for its two independently-optimized, independently-clipped actor/critic networks (Section 5.1c), and `train_buffered` for its buffer/minibatch-epoch update procedure. What's left is genuinely just running it to convergence: the script is verified to run correctly, but nobody has yet trained it on full adaptec1/ariane and compared the resulting HPWL to MaskPlace's published numbers. See the companion comparison doc for the full, up-to-date diff. |
...
- **Grouped optimizer** (added v4.3): `training/algorithm/split_optimizer.py`'s `make_grouped_optimizer()` — a different `optax.GradientTransformation` per named parameter group, each with its own optimizer state. Genuinely independent per-network optimization (e.g. actor/critic) without a second backward pass, as long as the groups share no parameters (Section 5.1c).
